...
case class PartialCity(name: String, latitude: Double, longitude: Double)

object PartialCity extends SQLSyntaxSupport[PartialCity] {
  override val tableName = "cities"
  ...
  def apply(p: SyntaxProvider[PartialCity])(rs: WrappedResultSet): PartialCity = apply(p.resultName)(rs)
  def apply(p: ResultName[PartialCity])(rs: WrappedResultSet): PartialCity = PartialCity(
    rs.string(p.name), rs.double(p.latitude), rs.double(p.longitude)
  )
}